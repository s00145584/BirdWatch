...
namespace BirdWatch
{
    public class CustomSeenListAdapter : BaseAdapter<String> {

        private Context mContext;
        private int id;
        private List<String> items;
        
        public CustomSeenListAdapter(Context context, int textViewResourceId, List<string> list)
        {
            mContext = context;
        ...
        public override View GetView(int position, View v, ViewGroup parent)
        {
            ...
            if (position % 2 == 1)
            {
                mView.SetBackgroundColor(Color.ParseColor("#578b41"));//row 2
            }
            else
            {
                mView.SetBackgroundColor(Color.ParseColor("#6dad52"));//row 1
            }
