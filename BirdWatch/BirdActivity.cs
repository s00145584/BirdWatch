...
namespace BirdWatch
{
    [Activity(Label = "Bird List")]
    public class BirdActivity : ListActivity
    {
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            using (SqlConnection connection = new SqlConnection(constring))
            {
                ...
                while (rdr.Read())
                {
                    birdList.Add(new Bird() { Name = rdr["Name"].ToString(), Picture = imageConvert(rdr["Picture"].ToString()), Description= rdr["Description"].ToString() });
                }
                connection.Close();
            }
